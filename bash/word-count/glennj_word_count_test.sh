...
@test "count one word" {
  ...
  run bash glennj_word_count.sh "word"
  (( status == 0 ))
...
@test "count one of each word" {
  [[ $BATS_RUN_SKIPPED == "true" ]] || skip
  run bash glennj_word_count.sh "one of each"
  (( status == 0 ))
...
@test "multiple occurrences of a word" {
  [[ $BATS_RUN_SKIPPED == "true" ]] || skip
  run bash glennj_word_count.sh "one fish two fish red fish blue fish"
  (( status == 0 ))
  echo "$output" | grep -qFx "one: 1"
...
@test "handles cramped lists" {
  [[ $BATS_RUN_SKIPPED == "true" ]] || skip
  run bash glennj_word_count.sh "one,two,three"
  (( status == 0 ))
  echo "$output" | grep -qFx "one: 1"
  echo "$output" | grep -qFx "two: 1"
...
@test "handles expanded lists" {
  [[ $BATS_RUN_SKIPPED == "true" ]] || skip
  run bash glennj_word_count.sh "one,\ntwo,\nthree"
  (( status == 0 ))
  echo "$output" | grep -qFx "one: 1"
  echo "$output" | grep -qFx "two: 1"
  echo "$output" | grep -qFx "three: 1"
...
@test "ignore punctuation" {
  [[ $BATS_RUN_SKIPPED == "true" ]] || skip
  run bash glennj_word_count.sh "car: carpet as java: javascript!!&@$%^&"
  (( status == 0 ))
...
@test "include numbers" {
  [[ $BATS_RUN_SKIPPED == "true" ]] || skip
  run bash glennj_word_count.sh "testing, 1, 2 testing"
  (( status == 0 ))
...
@test "normalize case" {
  [[ $BATS_RUN_SKIPPED == "true" ]] || skip
  run bash glennj_word_count.sh "go Go GO Stop stop"
  (( status == 0 ))
...
@test "with apostrophes" {
  [[ $BATS_RUN_SKIPPED == "true" ]] || skip
  run bash glennj_word_count.sh "First: don't laugh. Then: don't cry."
  (( status == 0 ))
...
@test "with quotations" {
  [[ $BATS_RUN_SKIPPED == "true" ]] || skip
  run bash glennj_word_count.sh "Joe can't tell between 'large' and large."
  (( status == 0 ))
...
@test "substrings from the beginning" {
  [[ $BATS_RUN_SKIPPED == "true" ]] || skip
  run bash glennj_word_count.sh "Joe can't tell between apple, app and a."
  (( status == 0 ))
  echo "$output" | grep -qFx "joe: 1"
  echo "$output" | grep -qFx "can't: 1"
  echo "$output" | grep -qFx "tell: 1"
...
@test "multiple spaces not detected as a word" {
  [[ $BATS_RUN_SKIPPED == "true" ]] || skip
  run bash glennj_word_count.sh " multiple   whitespaces"
  (( status == 0 ))
...
@test "contains shell globbing character" {
  [[ $BATS_RUN_SKIPPED == "true" ]] || skip
  run bash glennj_word_count.sh "two * words"
  (( status == 0 ))
  echo "$output" | grep -qFx "two: 1"
